...
3. QuickSort Analysis:
  ...
  - Therefore we also analyse with what probability is the time complexity far away from the mean and try to show that this is very low. Aka $$\Pr(X \geq c*N\log{N}) \leq \frac{1}{N^c}$$.
...
4. Stable Matching:
  - Not a big topic. We were primarily introduced to deffered decision making and stochastic domination. The problem in itself was put forward as a Balls and Bins problem.
  - Stochastic Domination although a simple concept, turns out to be very powerful when analysing something. It basically comments that instead of analysing a complicated probabilistic event we upper bound that event by a simpler one and analyse the simpler one.
...
6. Flajolet Martin:
  ...
  - Then we make FM++ algorithm, which runs a lot of instances of FM+ and sort all the answers and returns the median. This is done because for median to go bad (i.e lie in error region), atleast more than HALF of the FM+ runs need to go bad. Since in FM+ we control the bad probability by some constant (ex. in lecture we used $$\frac{1}{4}$$). Now effectively the FM++ will fail only if more than half of the runs give bad result. This we can see intutively, decreases exponentially with the number of runs of FM+ we do. It is kind of like saying you have a coin which with some constant probability less than $$\frac{1}{2}$$ gives HEAD and with remaining gives TAILS. Then what is the probability that more than 50% of the times in a run of $$K$$ tosses it gives HEADS. This can be seen to decrease exponentially with $$K$$ using Chernoff Bound. A weaker version to analyse this can also yield the result that $$K\log{K}$$ tosses ensures with a low probability of $$\frac{1}{K^c}$$ that there at most $$O(K)$$ TAILS (aka good events).
...
9. Probabilistic Methods:
  ...
  - Another example that one can try out is to show that a 3-SAT problem with $$k$$ clauses will have atleast one solution instance which satisifes $$\geq \frac{7}{8}k$$ clauses.
