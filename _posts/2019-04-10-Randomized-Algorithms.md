...
1. Inequalities:
  ...
  - We also exploit this kind of algebraic structure a lot in the course: $$(1 - 1/n)^{n * c \log{n}} \leq e^{c * \log{n}} \leq \frac{1}{n^c}$$, where c is a small positive integer.
...
6. Flajolet Martin:
  ...
  - Then we make FM++ algorithm, which runs a lot of instances of FM+ and sort all the answers and returns the median. This is done because for the median to go bad (i.e lie in error region), at least more than HALF of the FM+ runs need to go bad. Since in FM+ we control the bad probability by some constant (ex. in lecture we used $$\frac{1}{4}$$). Now effectively the FM++ will fail only if more than half of the runs give a bad result. This we can see intuitively, decreases exponentially with the number of runs of FM+ we do. It is kind of like saying you have a coin which with some constant probability less than $$\frac{1}{2}$$ gives HEAD and with remaining gives TAILS. Then what is the probability that more than 50% of the times in a run of $$K$$ tosses it gives HEADS. This can be seen to decrease exponentially with $$K$$ using Chernoff Bound. A weaker version to analyze this can also yield the result that $$K\log{K}$$ tosses ensure with a low probability of $$\frac{1}{K^c}$$ that there at most $$O(K)$$ TAILS (aka good events).
